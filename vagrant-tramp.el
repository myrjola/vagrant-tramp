...
(defun vagrant-tramp-ssh-to-box-cmd (box-id)
  (format "vagrant ssh %s" box-id))
...
(defun vagrant-tramp-list ()
  (with-temp-buffer
    ...
    (let* ((vm-list (subseq (split-string (buffer-string) "\n" t) 2))
           (first-empty-line-idx ( -elem-index " " vm-list)))
      (subseq vm-list 0 first-empty-line-idx))))

;; tramp completion functions take a file path argument where the file must
;; exist or the function is not called. we ignore the argument here and just use
;; `vagrant-tramp-list'
...
FILE argument is ignored."
  (mapcar (lambda (id-and-name)
            (list nil (first (split-string id-and-name
...
(defun vagrant-tramp-term (box)
  ...
  (let ((name (concat "vagrant ssh " box))
        (cmd (vagrant-tramp-ssh-to-box-cmd box)))
